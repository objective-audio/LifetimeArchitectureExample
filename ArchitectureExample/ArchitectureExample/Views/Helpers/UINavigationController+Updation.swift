...
extension UINavigationController {
    ...
    func updateViewControllers<Element: Equatable>(elements newElements: [Element],
                                                   fetching: (UIViewController) -> Element,
                                                   making: (Element) -> UIViewController?) {
        let oldElements = self.viewControllers.map(fetching)
